...
def send_data():
    ...
    #data = {'entry': {"Temperature": str(sensor.temperature()), "Humidity": str(sensor.humidity())},
    #       'date': str(datetime.datetime.now()) }
    data = {'value': str(sensor.humidity()),
            'date': datetime.datetime.now().strftime("%Y-%m-%d %H:%M")}
    headers = {'Content-Type': 'application/json'}
    r = requests.post('http://192.168.2.13:5000/data/api/v1.0/entry', data=json.dumps(data), headers=headers)
